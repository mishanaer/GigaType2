...
- On macOS, Type's `Fn`/`Globe` hotkey can conflict with the system action assigned to the same key, especially Dictation; symptoms may include Type not starting normally or only working through another microphone/headset. The assignment is exposed through the private `com.apple.HIToolbox` preference `AppleFnUsageType` (`0` = do nothing, `1` = change input source, `2` = emoji, `3` = Dictation). Treat nonzero values as occupied when choosing a new default, but handle missing or unknown values conservatively because this is not a stable public API, and never rewrite the macOS preference automatically. Type's `NSEvent.addGlobalMonitorForEvents` listener only observes the key and cannot suppress the macOS action.
