...
const MACOS_UNVERIFIED_PASTE_RETRY_DELAY_MS = 250;
...
class ClipboardManager {
  ...
  async _verifyMacOSPaste(text, verifyPaste) {
    try {
      const result = await verifyPaste({ text });
    ...
    try {
      ...
      if (platform === "darwin") {
        ...
        if (verifyBeforeRestore) {
          let verification = await this._verifyMacOSPaste(text, verifyPaste);
          if (!verification.inserted && verification.retryable) {
            this.safeLog("⚠️ Paste not verified, retrying once...");
            clipboard.writeText(text);
            await sleep(MACOS_UNVERIFIED_PASTE_RETRY_DELAY_MS);
            await this.pasteMacOS(null, options);
            verification = await this._verifyMacOSPaste(text, verifyPaste);
          }
  ...
  showAccessibilityDialog(testError) {
    ...
    if (isStuckPermission) {
      dialogMessage = `🔒 GigaType needs Accessibility permissions, but it looks like you may have OLD PERMISSIONS from a previous version.

❗ COMMON ISSUE: If you've rebuilt/reinstalled GigaType, the old permissions may be "stuck" and preventing new ones.

🔧 To fix this:
1. Open System Settings → Privacy & Security → Accessibility
2. Look for ANY old "GigaType" entries and REMOVE them (click the - button)
3. Also remove any entries that say "Electron" or have unclear names
4. Click the + button and manually add the NEW GigaType app
...
Would you like to open System Settings now?`;
      ...
      dialogMessage = `🔒 GigaType needs Accessibility permissions to paste text into other applications.
...
4. Add GigaType to the list and check the box
5. Restart GigaType
